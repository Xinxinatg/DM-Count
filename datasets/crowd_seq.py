...
class Base(data.Dataset):
    ...
    def train_transform(self, img, img_cross,keypoints):
        wd, ht = img.size
        ...
        img_cross = F.crop(img_cross, i, j, h, w)
        if len(keypoints) > 0:
        ...
        return self.trans(img),self.trans(img_cross), torch.from_numpy(keypoints.copy()).float(), st_size, torch.from_numpy(
...
class Crowd_sh(Base):
    ...
    def __getitem__(self, item):
        ...
        if int(name1)<30:
            name1=int(name1)+1
            name1="{0:0=5d}".format(name1)
            name1=name.split('_')[0]+'_'+name.split('_')[1]+'_'+name1
            path1=path.replace(name,name1)
        else:
            name1=int(name1)-1
            name1="{0:0=5d}".format(name1)
            name1=name.split('_')[0]+'_'+name.split('_')[1]+'_'+name1
            img_path1=img_path.replace(name,name1)
        img_cross=Image.open(img_path1).convert('RGB')
        keypoints = sio.loadmat(gd_path)['image_info'][0][0][0][0][0]

        if self.method == 'train':
            return self.train_transform(img,img_cross, keypoints)
        elif self.method == 'val':
            img = self.trans(img)
            img_cross = self.trans(img_cross)
            return img, len(keypoints), name
